...
namespace ProEventos.Application
{
    ...
    public async Task<PalestranteDto> AddPalestrantes(int userId, PalestranteAddDto model)
    {
        try
        {
            var Palestrante = this.mapper.Map<Palestrante>(model);
            ...
            if (await _palestrantePersist.SaveChangesAsync())
            {
                ...
                return this.mapper.Map<PalestranteDto>(PalestranteRetorno);
            }
            return null;
        }
        catch (Exception ex)
    ...
    public async Task<PalestranteDto> UpdatePalestrante(int userId, PalestranteUpdateDto model)
    {
        try{
            ...
            model.UserId = userId;
            this.mapper.Map(model, Palestrante);
    ...
    public async Task<PageList<PalestranteDto>> GetAllPalestrantesAsync(PageParams pageParams, bool includeEventos = false)
    {
        try
        {
            ...
            var resultado = this.mapper.Map<PageList<PalestranteDto>>(Palestrantes);
    ...
    public async Task<PalestranteDto> GetPalestranteByUserIdAsync(int userId, bool includeEvrntos = false)
    {
        try
        {
            var Palestrante = await _palestrantePersist.GetPalestranteByUserIdAsync(userId, includeEvrntos);
            if (Palestrante == null) return null;

            var resultado = this.mapper.Map<PalestranteDto>(Palestrante);
